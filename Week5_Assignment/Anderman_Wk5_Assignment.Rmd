...
#### Import CSV data
Read in the CSV and check its structure:

```{r read-csv, warning = FALSE}
delays <- read.csv("airline_delays.csv", stringsAsFactors = FALSE)
delays
str(delays)
```

#### Prepare `delays` data frame for tidying
Replicate each airline name twice to fill in values in empty strings and rename the first two columns of `delays`:

```{r prep-df}
delays[1] <- rep(delays[which(delays[1] != ""), 1], each = 2)
